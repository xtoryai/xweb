...
export const updateStores = ({ changes, savedEntries, savedAssets }) => {
  ...
  allEntries.update((entries) => {
    // Find the position of the first saved entry to preserve sort order
    const firstSavedIndex = entries.findIndex((e) => savedEntryIds.has(e.id));
    // Remove old versions of saved entries
    const filtered = entries.filter((e) => !savedEntryIds.has(e.id));
    // Re-insert saved entries at original position (or append for new entries)
    if (firstSavedIndex >= 0) {
      filtered.splice(firstSavedIndex, 0, ...savedEntries);
    } else {
      filtered.push(...savedEntries);
    }
    return filtered;
  });
