...
export const saveEntry = async ({ skipCI = undefined } = {}) => {
  ...
  // Re-fetch the first page from the API to restore correct sort order
  const backendSvc = get(backend);
  if (backendSvc?.fetchCollectionPage && collectionName) {
    const { key, order } = get(currentView).sort ?? {};
    backendSvc.fetchCollectionPage({
      ...
      // Silently ignore fetch errors — local state is already updated
    });
  }
