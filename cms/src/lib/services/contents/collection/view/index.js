...
import { allEntries, collectionPagination } from '$lib/services/contents';
...
export const collectionState = derived(
  [listedEntries, selectedCollection, collectionPagination],
  ([_listedEntries, _selectedCollection, _pagination]) => {
    if (_selectedCollection?._type === 'entry') {
      ...
      // Use pagination total when available, otherwise fall back to listedEntries length
      const totalEntries = _pagination[_selectedCollection?.name ?? '']?.total
        ?? _listedEntries.length;
      const remaining = quota < Infinity ? quota - totalEntries : Infinity;
...
selectedCollection.subscribe((collection) => {
  ...
  // Set default view type from collection config
  if (collection) {
    const savedView = get(entryListSettings)?.[collection.name];
    if (!savedView && collection.view_type === 'grid') {
      currentView.set({ type: 'grid' });
    }
  }
