...
<script>
  ...
  /** Guard against concurrent paste processing. */
  let pasting = false;
  ...
  const onPaste = async (event) => {
    ...
    // Guard against concurrent/re-entrant paste processing
    if (pasting) {
      return;
    }
    ...
    // No image file found — let the default/Lexical paste handler take over
    if (fileIndex === -1) {
      return;
    }

    // Image file detected — prevent the browser default and stop propagation
    // IMMEDIATELY and SYNCHRONOUSLY to block Lexical from seeing this event
    ...
    try {
      ...
      if (htmlIndex > -1) {
        // Handle pasted remote files: When a remote image is copied within the browser, both file
        // and HTML with `<img>` are added to the clipboard. Scrape the filename and alt text from
        // the HTML content
